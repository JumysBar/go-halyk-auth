...
func (s *MySecurityServer) revokeToken(userID int64) error {
	key := fmt.Sprintf("user:%d", userID)
	return s.redisConn.Del(key).Err()
}
...
func (s *MySecurityServer) AuthorizationCheck(ctx *fasthttp.RequestCtx) bool {
	// Checking auth
...
func (s *MySecurityServer) CheckAuthMiddleware(next fasthttp.RequestHandler) fasthttp.RequestHandler {
	...
	r.GET("/logout", func(ctx *fasthttp.RequestCtx) {
		...
		if err := s.revokeToken(user); err != nil {
